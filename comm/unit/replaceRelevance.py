...
def replace_random(param):
    ...
    esc_list = re.findall(pattern_esc, str(param))
    ...
    if len(esc_list):
        for each in esc_list:
            value = '${' + each.replace('\\', '') + '}'
            param = re.sub(pattern_esc, value, param, count=1)
...
if __name__ == '__main__':
    ...
    print('区分同形式变量：', replace(r'$\{unitCode}'))
